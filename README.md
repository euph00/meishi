...
preview falls back to text-only). Post bodies support a deliberately tiny
markup set — `**bold**`, `*italic*`, `{"pull": …}` standout quotes, and an
excerpt that doubles as the lede and preview description. The homepage
preview card is intentionally pending: it will reuse the finalized physical
namecard design.

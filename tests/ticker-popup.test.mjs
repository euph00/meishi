...
test('ticker data and rendered markup expose a semantic event programme', () => {
  ...
  assert.equal(content.ticker.length, 6);
  ...
  assert.equal((html.match(/class="ticker-dialog__event"/g) ?? []).length, 6);
  assert.deepEqual(
    [...html.matchAll(/<time[^>]+datetime="([^"]+)"/g)].map((match) => match[1]),
    ['2026-09-20', '2026-09-21', '2026-11-07', '2026-12-27', '2026-12-29', '2026-12-31', '2027-02-20']
  );
...
test('ticker opens and dismisses the event programme accessibly', async () => {
  ...
  assert.equal(await dialog.locator('.ticker-dialog__event').count(), 6);

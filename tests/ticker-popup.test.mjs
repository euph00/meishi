...
test('past events are canonical, numeric, and rendered most-recent-first', () => {
  ...
  assert.deepEqual(content.pastEvents, [
    { title: '初後夜祭 in 岩手（DJイベント）', date: '2026-09-22', endDate: '2026-09-23' },
  ...
  assert.equal((html.match(/class="past-events__event"/g) ?? []).length, 3);
  assert.match(html, /初後夜祭 in 岩手（DJイベント）/);
  assert.equal(content.ticker.some((event) => event.title.includes('初後夜祭 in 岩手')), false);
